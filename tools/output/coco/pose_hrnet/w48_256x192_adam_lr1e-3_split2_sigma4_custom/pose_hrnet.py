...
class PoseHighResolutionNet(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        if self.coord_representation == 'simdr' or self.coord_representation == 'sa-simdr':
            ##To enhance the precision of localization, we introduce a splitting factor k(≥ 1) 
            ##and rescale the ground-truth coordinate into a new coordinate.
    ...
    def forward(self, x):
        ...
        ##x_.shape为[B, 17, 16, 16]，分辨率下采样了4倍
        ...
        if self.coord_representation == 'heatmap':
            ...
            ##pred_x.shape为[B, 17, 192]
            pred_x = self.mlp_head_x(x)
            ##pred_y.shape为[B, 17, 192]
